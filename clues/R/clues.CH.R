...
clues.CH<-function(y, n0=5, alpha=0.05, eps=1.0e-4, itmax=10, 
  K2.vec=n0, CH2=-3, disMethod="Euclidean", 
  plotFlag=FALSE, plot.dim=c(1,2), quiet=FALSE)
{
  ...
  # first pass
  second<-FALSE
  res<-ChooseK.CH(y, y2=y, n0, alpha, eps, itmax, second, K2.vec, CH2, 
...
#      if(!quiet) { cat("pos1=",pos1, " g1=", g1, "\n") }
    }
    if(pos2<len3)
    { g2<-res$g.vec[pos2+1]
      K22<-res$K.vec[pos2+1]
#      if(!quiet) { cat("pos2=", pos2, " g2=", g2, "\n") }
    }
...
#      }
      # second pass
      second<-TRUE
      res2<-ChooseK.CH(y, y2=res$y.old1, n0, alpha, eps, itmax, second, 
        K2.vec, CH2, disMethod, plotFlag, plot.dim, quiet)
      if(res2$myupdate == T) # update the final partition
      { CH2<-res2$CH
        res<-res2
      }
    }
...
#      }
      # second pass
      second<-TRUE
      res3<-ChooseK.CH(y, y2=res$y.old2, n0, alpha, eps, itmax, second, 
        K2.vec, CH2, disMethod, plotFlag, plot.dim, quiet)
      if(res3$myupdate == T) { res<-res3 }
    }
  }
  return(res)
}
...
# first pass
# if second=F, then it is the first pass
# if second=T, then it is the second pass
# y --- original data
# y2 --- data used to do shrinking and clustering
ChooseK.CH<-function(y, y2, n0=5, alpha=0.05, eps=1.0e-4, itmax=10, 
  second=FALSE, K2.vec=n0, CH2=-3, disMethod="Euclidean",
  plotFlag=FALSE, plot.dim=c(1,2), quiet=FALSE)
{
  ...
  res<-.Fortran("choosekch", dat, dat2, nObs, nObs1, nVars, nClusters0, 
     alpha, eps,
     ITMAX, second, nNeiVec2, CH2, quiet,disMethod2,
     CHFinal=CHFinal, 
     memFinal=memFinal, 
...
Get.CH<-function(y, mem, disMethod="Euclidean")
{
  disMethod=match.arg(disMethod, c("Euclidean", "1-corr"))
  ...
  #output
  CH<-0
  storage.mode(CH)<-"double"

  res<-.Fortran("chindex", y, n, p, mem, g, size, disMethod2, CH=CH,
    PACKAGE="clues") 
